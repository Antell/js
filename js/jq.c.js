...
// (高亮注释 Techenicolor 2011-2-11 21:12:33 空白)保存jQuery的关键字, 免得被重写后没法恢复.
...
//callback将会作为elem的方法来调用, 
     ...
     * 为了能够更好地理解domManip函数的功能, 
     可以结合jQuery.fn.append来说明此函数的作用: 



     * 在append函数中有这样的代码:return this.domManip(arguments, 
...
//在IE中,如果需要在table中操作tr(如插入一个tr), 
                IE要求你在tbody内进行操作.




// 以下的if就是说要在tbody内进行操作, 
...
// A method for quickly swapping in/out CSS properties to get 
     ...
     * 因此先将元素的visibilty设置为 position: "absolute",
...
// 就需要对结果进行'修剪'.因为会有border和padding的问题. 
        用具体看下面的代码.
...
// value不是 null，说明处理成功，把它加入新的数组里面去 
                                ...
                                //可以在下面看到一些挺有趣的结果:比如写着是IE但可能是opera浏览器,写着M
                                ozilla但也有有可能是IE浏览器...
...
//经过fn的处理之后,如果有selector就是使用selector进行过滤. 
                                ...
                                */
                                ...
                                //each函数给callback函数(也就是下面这个函数)传入属性的名称(name),以及
...
// Remove any remaining nodes 
                                ...
                                //这个就是each所调用的callback函数.它将上述每一个函数包装一下,然后把他
...
* 第二类是"异曲同工"的事件,在这里就是mouseenter和mouseleave. 
...
* 如果在非IE的浏览器中要求绑定mouseenter事件, 那不管, 
...
{name: elem.name, value: val};
                                        ...
                                        */
                                        ...
                                        个jQuery 对象又具有了ajax 方面事件的监听能力-----------------4877
                                        /*
                                        * 以下代码为
...
// 将"=?"替换为jQuery在jsonp变量里面定义的callback名称, 
...
// 
offsetParent不是body或者html 


// offsetParent的position 
== 'static' 
